...
namespace Fusion.Engine.Audio
{
    public sealed class SoundEffect : IDisposable
    {
        ...
        internal DataStream		dataStream;
		...
		public SoundEffect( SoundSystem device, byte[] buffer, int offset, int bytesCount, int sampleRate, AudioChannels channels, int loopStart, int loopLength )
        {
			this.device	=	device;
        ...
        private void Initialize(WaveFormat format, byte[] buffer, int offset, int count, int loopStart, int loopLength)
        {
            ...
            dataStream = DataStream.Create<byte>(buffer, true, false);
