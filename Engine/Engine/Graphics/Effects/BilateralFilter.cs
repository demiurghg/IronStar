	...
	internal class BilateralFilter : RenderComponent {
		...
		public void RenderBilateralFilter ( Camera camera, RenderTarget2D target, RenderTarget2D temp, ShaderResource depth, float depthFactor, float colorFactor )
		{
			...
			using( new PixEvent("BilateralFilter") ) 
			{
				...
				device.ComputeResources[0]    =   target;
				device.ComputeResources[1]    =   depth;

				device.ComputeConstants[ regParams	]	=	cbuffer;
				device.ComputeConstants[ regCamera	]	=	camera.CameraData;

				device.SetComputeUnorderedAccess( 0, temp.Surface.UnorderedAccess );
				...
				device.ComputeResources[0]    =   temp;
				device.ComputeResources[1]    =   depth;
