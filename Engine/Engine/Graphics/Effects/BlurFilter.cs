	...
	internal class BlurFilter : RenderComponent {
		...
		public void GaussBlur( RenderTarget2D target, RenderTarget2D temp, int mipLevel )
		{
			...
			using( new PixEvent("GaussBlur") ) {
				...
				device.ComputeSamplers[0]		=	SamplerState.LinearClamp;
				device.ComputeResources[0]	=	target.GetShaderResource( mipLevel );
				device.SetComputeUnorderedAccess( 0, temp.GetSurface( mipLevel ).UnorderedAccess );
				...
				device.ComputeSamplers[0]		=	SamplerState.LinearClamp;
				device.ComputeResources[0]	=	temp.GetShaderResource( mipLevel );
				device.SetComputeUnorderedAccess( 0, target.GetSurface( mipLevel ).UnorderedAccess );
