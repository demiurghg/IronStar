		...
		public void Render ( StereoEye stereoEye, Camera camera, HdrFrame hdrFrame )
		{
			...
			using (new PixEvent("HDAO Render")) {
				...
				using ( new PixEvent( "Interleave" ) ) {
					...
					device.ComputeResources[0]    =   depthBuffer;
					
					device.SetComputeUnorderedAccess( 0, depthSliceMap0.Surface.UnorderedAccess );
					device.SetComputeUnorderedAccess( 1, depthSliceMap1.Surface.UnorderedAccess );
					device.SetComputeUnorderedAccess( 2, depthSliceMap2.Surface.UnorderedAccess );
					device.SetComputeUnorderedAccess( 3, depthSliceMap3.Surface.UnorderedAccess );
				...
				using ( new PixEvent( "HDAO Pass" ) ) {
					...
					for (int i=0; i<4; i++) {
						...
						device.ComputeResources[0]	=	slices[i];

						device.SetComputeUnorderedAccess( 0, occlusionMap.Surface.UnorderedAccess );
