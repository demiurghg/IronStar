...
namespace BEPUrender.Lines
{
    ...
    public class DisplayEllipseSwingLimit : SolverDisplayObject<EllipseSwingLimit>
    {
        ...
        public override void Update()
        {
            ...
            float angleIncrement = 4 * MathUtil.Pi / limitLines.Length; //Each loop iteration moves this many radians forward.
            for (int i = 0; i < limitLines.Length / 2; i++)
            {
                ...
                pointToPreviousPoint.PositionA = MathConverter.Convert(LineObject.ConnectionB.Position) +
                                                 ...
                                                 Vector3.TransformNormal(MathConverter.Convert(LineObject.Basis.PrimaryAxis), Matrix.RotationAxis(rotationAxis, angle));
