...
namespace BEPUrender.Lines
{
    ...
    public class DisplayRevoluteLimit : SolverDisplayObject<RevoluteLimit>
    {
        ...
        public override void Update()
        {
            ...
            Vector3 upwardsOffset = Vector3.TransformNormal(MathConverter.Convert(LineObject.Basis.XAxis), Matrix.RotationAxis(MathConverter.Convert(LineObject.Basis.PrimaryAxis), LineObject.MaximumAngle));
            Vector3 topRightPosition = right + upwardsOffset;
            Vector3 topLeftPosition = left + upwardsOffset;

            Vector3 downwardsOffset = Vector3.TransformNormal(MathConverter.Convert(LineObject.Basis.XAxis), Matrix.RotationAxis(MathConverter.Convert(LineObject.Basis.PrimaryAxis), LineObject.MinimumAngle));
